...
"""
word2vec的cbow + negative sample实现
"""
...
flags.DEFINE_string("model_path", "cbow.pkl", "模型文件")
flags.DEFINE_string("log_path", "logs", "log记录位置")
FLAGS = flags.FLAGS

###############################################################################
class CbowModel(object):
    """ cbow + negative sample 模型 """
...
def main(_):
    ...
    model = CbowModel(FLAGS.learning_rate, vocab_size, FLAGS.emb_size, 
    ...
    for epoch in range(FLAGS.epoch_size + 1):
        for line in fp:
            ...
            # 打印 窗口向量 和 目标向量
            x_emb, context_emb, y_emb = \
                sess.run([model.embed, model.context_emb, model.ys], feed_dict=feed_dict)
