...
"""
word2vec: skipgram + negative sample 模型
"""
...
flags.DEFINE_string("model_path", "skip.pkl", "模型文件")
flags.DEFINE_string("log_path", "logs", "log记录位置")
FLAGS = flags.FLAGS

###############################################################################
class SkipgramModel(object):
    """ skipgram + negative sample 模型 """
    ...
    def __init__(self, lr, vocab_size, emb_size, context_window, num_sampled, 
        ...
        # 输入数据
        self.xs = tf.placeholder(tf.int32, shape=[None], name="xs")
        ...
        with tf.device("/cpu:0"):
            embed = tf.nn.embedding_lookup(embeddings, self.xs)
...
def main(_):
    ...
    model = SkipgramModel(FLAGS.learning_rate, vocab_size, FLAGS.emb_size, 
    ...
    for epoch in range(FLAGS.epoch_size + 1):
        for line in fp:
            step += 1
            # 输入训练数据 并 更新网络
            xs, ys = reader.gen_skipgram_data(line)
